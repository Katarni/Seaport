...
class DrawableScheduleItem : kat::Div {
 public:
    explicit DrawableScheduleItem(const ScheduleItem& item, sf::RenderWindow* parent) : kat::Div(parent) {
        ...
        labels_.resize(5);
        vals_.resize(5);
        std::vector<float> lbls_x = {43, 207, 300, 30, 237},
                            lbls_width = {48, 48, 72, 84, 84},
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            labels_[i]->setWidth(lbls_width[i]);
