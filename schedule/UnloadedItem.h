...
class UnloadedItem : public kat::Div {
 public:
    explicit UnloadedItem(Ship* item, sf::RenderWindow* parent) : kat::Div(parent) {
        resize(472, 145);
        setBorderRadius(15);
        ...
        labels_.resize(4);
        vals_.resize(5);
        std::vector<float> lbls_x = {32, 238, 98, 104},
                lbls_width = {84, 84, 186, 156},
                lbls_y = {44, 44, 75, 107};
        std::vector<std::string> lbls_text = {"arrival", "waiting", "start unloding", "unloding time"};
        ...
        vals_[1]->setData(intToTime(item->getArrival()));
        vals_[2]->setData(intToTime(item->getWaitingTime()));
        vals_[3]->setData(intToTime(item->getStartOfUnloading()));
