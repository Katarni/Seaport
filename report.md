...
+ 'S' - Schedule, кнопка открывает и закрывает панель с расписание
+ 'results'  - кнопка открывает окно с результатами
  
На первой и второй дорогах представлены две фуры. Первая показывает текущее время, вторая же содержит элементы управления: перемотать на день раньше, поставить на паузу / запустит моделирование, перемотать на день дальше.
### Окно результатов
